...
class ERDiagramParser:
    ...
    def _parse_entities(self):
        ...
        entities_match = re.search(r'##\s+.+?Entities(.*?)(?=\n##\s+|\Z)', 
    ...
    def _parse_relationships(self):
        ...
        rel_sections = re.finditer(r'###\s+.+?Relationships\n(.*?)(?=\n###|\n##\s+|\Z)', 
                                  self.content, re.DOTALL)
        
        # Also try to find main ## Relationships section (may have emoji)
        main_rel_match = re.search(r'##\s+.+?Relationships(.*?)(?=\n##\s+|\Z)', 
                                   self.content, re.DOTALL)
...
class ERDiagramGenerator:
    ...
    def to_plantuml(self) -> str:
        ...
        for rel in self.relationships:
            ...
            lines.append(f"{rel.from_entity} {card_str} {rel.to_entity} : {rel.description}")
...
def main():
    ...
    parser.add_argument(
        'spec_file',
        help='Path to OpenSpec data model file'
    )
    parser.add_argument(
        '--format',
