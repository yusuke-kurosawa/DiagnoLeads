        ...
        for fk in fks:
            if column in fk["constrained_columns"] and fk["referred_table"] == parent_table:
                ...
                if actual_ondelete and actual_ondelete != on_delete:
                    self.errors.append(f"{child_table}.{column} -> {parent_table}: 期待: ondelete={on_delete}, 実際: ondelete={actual_ondelete}")
                break

        if not found:
            self.errors.append(f"{child_table}.{column} -> {parent_table} の外部キー制約が存在しません")
    ...
    def validate_orphan_records(self):
        ...
        for child_table, fk_column, parent_table, parent_id in orphan_checks:
            try:
                ...
                if orphan_count > 0:
                    self.errors.append(f"{child_table} に {orphan_count} 件の孤児レコードが存在します (参照先: {parent_table}.{parent_id})")
                    total_orphans += orphan_count
            except Exception as e:
                self.warnings.append(f"{child_table} の孤児レコードチェック失敗: {e}")

        if total_orphans == 0:
            print("  ✓ 孤児レコードは検出されませんでした\n")
        else:
    ...
    def validate_unique_constraints(self):
        ...
        for constraint in self.schema_constraints["unique_constraints"]:
            ...
            try:
                ...
                if duplicates:
                    self.errors.append(f"{table} の {cols_str} に {len(duplicates)} 件の重複があります")
                checked_count += 1
            except Exception as e:
    ...
    def validate_check_constraints(self):
        ...
        for constraint in self.schema_constraints["check_constraints"]:
            ...
            try:
                ...
                if violations > 0:
                    self.errors.append(f"{table} で {violations} 件のチェック制約違反: {constraint_name}")
                checked_count += 1
            except Exception as e:
        ...
        for idx_type in ["performance", "uniqueness"]:
            ...
            for idx in self.schema_constraints["indexes"][idx_type]:
                ...
                try:
                    ...
                    if not found:
                        self.warnings.append(f"{table} に推奨インデックスが存在しません: {', '.join(columns)}")
                    checked_count += 1
                except Exception as e:
        ...
        for mapper in Base.registry.mappers:
            ...
            for rel in mapper.relationships:
                ...
                if rel.back_populates is None and rel.backref is None:
                    self.warnings.append(f"{model_class.__name__}.{rel.key} に back_populates または backref が設定されていません")
                checked_count += 1
