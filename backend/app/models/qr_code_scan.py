...
class QRCodeScan(Base):
    ...
    user_agent: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Full user agent string"
    )
    device_type: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Device type: mobile, tablet, desktop"
    )
    os: Mapped[str | None] = mapped_column(
        String(100), comment="Operating system: iOS, Android, Windows, etc."
    )
    browser: Mapped[str | None] = mapped_column(
        String(100), comment="Browser: Safari, Chrome, Firefox, etc."
    )
    ...
    latitude: Mapped[float | None] = mapped_column(
        Float, comment="Approximate latitude"
    )
    longitude: Mapped[float | None] = mapped_column(
        Float, comment="Approximate longitude"
    )
    ...
    lead_created: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, comment="Whether a lead was created"
    )
    ...
    session_id: Mapped[str | None] = mapped_column(
        String(255), comment="Session ID from cookie/localStorage"
    )
