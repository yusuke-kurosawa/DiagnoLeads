...
class TestScanTracking:
    ...
    async def test_link_lead_to_scan(
        ...
        response = await client.put(
            f"/api/v1/scans/{scan.id}/lead",
            params={"lead_id": str(lead.id)}
        )
...
class TestQRAnalytics:
    ...
    async def test_get_analytics(
        ...
        response = await client.get(
            f"/api/v1/qr-codes/{qr.id}/analytics",
    ...
    async def test_analytics_with_custom_days(
        ...
        # Request last 7 days
        response = await client.get(
            f"/api/v1/qr-codes/{qr.id}/analytics?days=7",
            headers=auth_headers
        )
    ...
    async def test_analytics_not_found(
        ...
        response = await client.get(
            f"/api/v1/qr-codes/{fake_id}/analytics",
            headers=auth_headers
        )
