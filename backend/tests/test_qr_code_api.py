...
async def test_tenant(db: AsyncSession) -> Tenant:
    """Create test tenant"""
    tenant = Tenant(id=uuid4(), name="Test Company", slug="test-company", plan="pro")
    db.add(tenant)
    await db.commit()
...
class TestQRCodeCRUD:
    ...
    async def test_list_qr_codes(
        ...
        # List QR codes
        response = await client.get("/api/v1/qr-codes", headers=auth_headers)
    ...
    async def test_get_qr_code(
        ...
        # Get QR code
        response = await client.get(f"/api/v1/qr-codes/{qr.id}", headers=auth_headers)
